...
SELECT * FROM MEMBER JOIN DEPARTMENT USING (DEPARTMENT_KEY) LEFT JOIN JOB USING(JOB_KEY) WHERE  ORDER BY JOB_KEY;
...
JOIN POST_MESSAGE_SEND pms ON PMF.POST_MSG_KEY =PMS.POST_MSG_SEND_KEY WHERE PMF.POST_MSG_KEY='193';
		...
		SELECT * FROM MEMBER JOIN DEPARTMENT USING (DEPARTMENT_KEY) LEFT JOIN JOB USING(JOB_KEY) WHERE MEMBER_NAME!='관리자' AND MEMBER_NAME!='test' ORDER BY JOB_KEY;
