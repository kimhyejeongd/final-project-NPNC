...
ORDER BY ER_FOLDER_GROUP DESC ,
	...
	SELECT * FROM CHATTING_GROUP cg  LEFT JOIN MEMBER USING(MEMBER_key)
	WHERE member_name ='강진주';
...
SELECT * 
		...
		WHERE cg.MEMBER_key = '1' AND CG.CHAT_GROUP_STATUS='Y'
			ORDER BY COALESCE(chat_msg_time, cg.CHAT_GROUP_DATE) DESC;
		...
		SELECT ef.ER_FORM_KEY,
				ef.ER_FORM_NAME,
				ef.ER_FORM_CREATER,
				ef.ER_FORM_CREATE_DATE,
				ef.ER_FORM_FOLDER_KEY,
				ef.ER_FORM_YN,
				ef.ER_FORM_UPDATER,
				ef.ER_FORM_UPDATE_DATE,
				ef.ER_FORM_ORDERBY,
				ef.ER_DOC_FILENAME,
				m.MEMBER_NAME,
				d.DEPARTMENT_NAME,
...
SELECT * FROM JOB;
	...
	SELECT ef.ER_FORM_KEY,
				...
				ef.ER_FORM_UPDATE_DATE,
				ef.ER_FORM_ORDERBY,
				ef.ER_DOC_FILENAME,m.MEMBER_KEY,
				m.DEPARTMENT_KEY,
				m.JOB_KEY,
				m.MEMBER_ID,
				m.MEMBER_PW,
		...
		LEFT JOIN MEMBER m ON ER_FORM_UPDATER = member_key 
		LEFT JOIN DEPARTMENT d ON m.DEPARTMENT_KEY = d.DEPARTMENT_KEY 
		LEFT JOIN JOB J ON  m.JOB_KEY =j.JOB_KEY		
		WHERE ER_FORM_FOLDER_KEY=1;
		
	SELECT * FROM ER_STORAGE es ;
	SELECT * FROM ER_STORAGE_FOLDER esf ;
	SELECT * FROM ER_FOLDER ef ;
	SELECT * FROM ER_FORM_FOLDER eff ;
	SELECT * FROM ER_FORM eff ;
	SELECT * FROM DEPARTMENT;
	
SELECT * FROM CHATTING_GROUP cg WHERE cg.CHAT_ROOM_KEY =205;

SELECT * FROM CHATTING_MESSAGE cm  WHERE CHAT_ROOM_KEY =205;
...
UPDATE MEMBER  SET member_id= 999 WHERE MEMBER_KEY =999;

    	INSERT INTO CHATTING_MESSAGE
    	VALUES (
    		SEQ_CHAT_MESSAGE_KEY.NEXTVAL,
    		1,
    		-1,
    		'dqwddw',
    		SYSDATE,
